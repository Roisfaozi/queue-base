...
test.describe("RBAC Flow - End to End", () => {
  ...
  test("should create a resource and manage permissions", async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator('button[role="tab"]:has-texttext("Matrix View")')).toBeVisible();
      ...
      const isCheckedBefore = (await userRoleSwitch.getAttribute("aria-checked")) === "true";
      await userRoleSwitch.click({ force: true });

      // Verify toggle state changed
      const expectedValue = isCheckedBefore ? "false" : "true";
      await expect(userRoleSwitch).toHaveAttribute("aria-checked", expectedValue, {
        timeout: 10000,
      });
