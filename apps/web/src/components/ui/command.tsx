...
>(({ className, ...props }, ref) => (
  <div className="flex items-center border-b px-3" cmdk-input-wrapper="">
    ...
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        "placeholder:text-muted-foreground flex w-full bg-transparent py-3 text-sm outline-hidden disabled:cursor-not-allowed disabled:opacity-50",
        "h-[var(--input-height)]",
        className,
      )}
      {...props}
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Item
    ...
    className={cn(
      ...
      "[data-density=compact]:py-1 [data-density=compact]:text-xs", // Density awareness
      className,
    )}
    {...props}
  />
