	...
	tests := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T, repo repository.AccessRepository, db *gorm.DB, ctx context.Context) {
				...
				db.Create(&endpoint)
				ar := entity.AccessRight{ID: "ar-preload", Name: "Preload Test", Endpoints: []entity.Endpoint{endpoint}}
				db.Create(&ar)
	...
	tests := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T, repo repository.AccessRepository, db *gorm.DB, ctx context.Context) {
				...
				db.Create(&endpoint)
				ar := entity.AccessRight{ID: "ar-find", Name: "Find Test", Endpoints: []entity.Endpoint{endpoint}}
				db.Create(&ar)
...
func TestAccessRepository_UnlinkEndpointFromAccessRight(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Negative - Unlink Non-existent Access Right",
			run: func(t *testing.T, repo repository.AccessRepository, db *gorm.DB, ctx context.Context) {
		...
		{
			name: "Negative - Unlink Non-existent Endpoint",
			run: func(t *testing.T, repo repository.AccessRepository, db *gorm.DB, ctx context.Context) {
				endpoint := entity.Endpoint{ID: "unlink-ep-missing-endpoint", Path: "/api/unlink", Method: "GET"}
				require.NoError(t, db.Create(&endpoint).Error)
				ar := entity.AccessRight{ID: "unlink-ar-missing-endpoint", Name: "Unlink Test Missing Endpoint"}
				require.NoError(t, db.Create(&ar).Error)
				err := repo.UnlinkEndpointFromAccessRight(ctx, "unlink-ar-missing-endpoint", "non-existent-ep")
				require.NoError(t, err)
			},
		},
		{
			name: "Edge - Unlink with Empty IDs",
			run: func(t *testing.T, repo repository.AccessRepository, db *gorm.DB, ctx context.Context) {
		...
		{
			name: "Vulnerability - SQL Injection attempt in ID",
			run: func(t *testing.T, repo repository.AccessRepository, db *gorm.DB, ctx context.Context) {
