...
func newSignageTestDB(t *testing.T) *gorm.DB {
	...
	require.NoError(t, db.Exec(`
		...
		CREATE TABLE services (
			...
			audio_id TEXT,
			audio_en TEXT,
			narrative_instruction_id TEXT,
			narrative_instruction_en TEXT
		);
...
func seedSignageTestData(t *testing.T, db *gorm.DB) {
	...
	stmts := []string{
		...
		`UPDATE services SET audio_id='audio-1', audio_en='audio-1-en', narrative_instruction_id='narr-1', narrative_instruction_en='narr-1-en' WHERE id='svc-1'`,
...
func TestSignageUseCase_GetMe(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "Negative_UnauthorizedWhenClientMissing",
			clientID:    "",
			expectedErr: exception.ErrUnauthorized,
		},
		{
			name:        "Negative_NotFoundWhenClientInactiveOrAbsent",
			clientID:    "missing",
			expectedErr: exception.ErrNotFound,
		},
		{
			name:     "Positive_ReturnsFullBoundSignageProfile",
			clientID: "signage-1",
			expectedModel: &model.SignageMeResponse{
				ClientID:               "signage-1",
				TenantID:               "t-1",
				...
				NarrativeInstructionEN: "narr-1-en",
			},
		},
		{
			name:     "Edge_FallsBackToTenantBrandingWhenBranchEmpty",
			clientID: "signage-4",
			expectedModel: &model.SignageMeResponse{
				ClientID:    "signage-4",
				TenantID:    "t-1",
...
func TestSignageUseCase_GetCurrentCalls(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "Negative_BadRequestWhenTenantOrBranchMissing",
			ctx:         context.Background(),
			clientID:    "signage-1",
			expectedErr: exception.ErrBadRequest,
		},
		{
			name:        "Vulnerability_ForbiddenWhenContextCrossesClientBinding",
			ctx:         newSignageCtxFor("t-1", "b-2"),
			clientID:    "signage-1",
			expectedErr: exception.ErrForbidden,
		},
		{
			name:     "Positive_FiltersByBranchServiceAndCounterBinding",
			ctx:      newSignageCtx(),
			clientID: "signage-1",
			expectedRows: []model.SignageCurrentCallResponse{
				{
		...
		{
			name:     "Edge_FiltersOnlyByBranchServiceWhenCounterAbsent",
			ctx:      newSignageCtx(),
			clientID: "signage-2",
			expectedRows: []model.SignageCurrentCallResponse{
				{
		...
		{
			name:     "Edge_ReturnsAllCurrentCallsWhenNoServiceBinding",
			ctx:      newSignageCtx(),
			clientID: "signage-3",
			expectedRows: []model.SignageCurrentCallResponse{
				{
...
func TestSignageUseCase_GetQueues(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "Positive_FiltersPendingQueuesByBranchService",
			ctx:      newSignageCtx(),
			clientID: "signage-1",
			expectedRows: []queueModel.QueueResponse{
				{
		...
		{
			name:         "Edge_ReturnsEmptyWhenNoPendingQueuesForServiceBinding",
			ctx:          newSignageCtx(),
			clientID:     "signage-2",
			expectedRows: []queueModel.QueueResponse{},
		},
